...
from src.data.datasets_stage3 import Stage3TrainDataset, Stage3InferenceDataset


class Stage3DataModule(pl.LightningDataModule):
	def __init__(
			self,
			...
			predict_l_dendrite_path: str = None,
			predict_necks_path: str = None,
			area_path: str = None,
	...
	def setup(self, stage: Optional[str] = None):
		...
		if stage == "fit" or stage is None:
			...
			self.train_ds = Stage3TrainDataset(self.hparams.train_patches_dir)
			self.val_ds = Stage3TrainDataset(self.hparams.val_patches_dir)
		...
		if stage == "predict":
			...
			if not self.hparams.predict_l_dendrite_path:
				raise ValueError("Stage 2 inference requires predict_l_dendrite_path!")

			# Здесь используется датасет, который режет TIFF на лету
			self.predict_ds = Stage3InferenceDataset(
				tiff_path=self.hparams.predict_tiff_path,
				current_scale=self.hparams.current_scale,
				l_dendrite_path=self.hparams.predict_l_dendrite_path,
				necks_path=self.hparams.predict_necks_path,
	...
	def predict_dataloader(self):
		return DataLoader(
			self.predict_ds,
			batch_size=2,  # Для инференса по одному патчу (или больше, если влезет)
			num_workers=self.hparams.num_workers,
			shuffle=False,
			persistent_workers=self.hparams.num_workers > 0
		)