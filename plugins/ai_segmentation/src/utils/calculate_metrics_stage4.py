...
def create_colored_mask(mask1: np.ndarray, mask2: np.ndarray) -> np.ndarray:
    ...
    # red = np.array([236, 126, 166], dtype=np.uint8)
    # white = np.array([255, 255, 255], dtype=np.uint8)    # белый фон
    # green = np.array([158, 214, 132], dtype=np.uint8)
    # blue = np.array([88, 144, 230], dtype=np.uint8)

    rgb_image = np.zeros((*mask1.shape, 3), dtype=np.uint8)

    rgb_image[intersection > 0] = white
    rgb_image[fn > 0] = red
    rgb_image[fp > 0] = blue

    # rgb_image = np.zeros((*mask1.shape, 3), dtype=np.uint8) + white

    # rgb_image[intersection > 0] = green
    # rgb_image[fn > 0] = red
    # rgb_image[fp > 0] = blue
...
def make_report(json: dict, output_folder: str) -> dict:
    ...
    for obj in json["data"]:
        ...
        # segm_dendr[pred_img == 127] = 1
        
        # segm_spines = np.zeros_like(pred_img)
        # segm_spines[pred_img == 254] = 2

        # binarization = segm_dendr + segm_spines


        segm_dendr_raw = imread(obj["result dendrites"])
        segm_spines_raw = imread(obj["result spines"])
        if area is not None:
            segm_dendr_raw = segm_dendr_raw * area
            segm_spines_raw = segm_spines_raw * area
        segm_dendr = np.zeros_like(segm_dendr_raw)
        segm_dendr[segm_dendr_raw >0] = 1
        segm_spines = np.zeros_like(segm_spines_raw)
        segm_spines[segm_spines_raw >0] = 2  
        binarization = segm_dendr + segm_spines
...
if __name__ == "__main__":
    ...
    with open(f"{args.output_folder}/metrics_report1.json", "w") as f:
