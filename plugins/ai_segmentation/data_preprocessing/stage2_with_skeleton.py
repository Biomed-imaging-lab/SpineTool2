...
def split_and_save(img_c, l_dendrite, isi, mask, skeleton, g_norm, name, angle, start_pos, output_dir):
...
def process_item(item, output_dir, is_train_mode, base_path, preds_dir):
    ...
    if scale:
        ...
        skeleton = scale_image(skeleton.astype(np.uint8), scale)
    ...
    for angle in angles:
        if angle != 0:
            ...
            img_rot, l_dendrite_rot, isi_rot, mask_rot, skeleton_rot = img_c_raw, l_dendrite_raw, i_si_raw, mask, skeleton
            
        g_norm = normalize_minmax(img_rot)    
        for pos in POSITIONS:
            if not is_train_mode and pos != [0,0,0]: continue
            saved = split_and_save(img_rot, l_dendrite_rot, isi_rot, mask_rot, skeleton_rot, g_norm, name, angle, pos, volume_output_dir)
            total_saved += saved
