...
from application.qt_jsonschema_form.signal import Signal
from application.qt_jsonschema_form.utils import (
    is_concrete_schema,
    iter_layout_widgets,
    state_property,
)
from application.widgets.qt_keyboard_settings import ShortcutViewer
...
class ShortcutsWidget(SchemaWidgetMixin, ShortcutViewer):
    @state_property
    def state(self) -> OrderedDict[str, ShortcutsHandler]:
        return self.value()
    ...
    @state.setter
    def state(self, state: OrderedDict[str, ShortcutsHandler]):
    ...
    def configure(self):
        ...
        for item in options:
            if isinstance(item, (tuple, list)) and len(item) == 2:
                ...
                opt, data = str(item), item
            self.addItem(opt, data)
