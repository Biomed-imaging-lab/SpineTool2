...
class QtConfirmRestoreSettings(QDialog):
    def __init__(self, parent) -> None:
        super().__init__(parent)
        cancel_btn = QtPushButton("cancel")
        close_btn = QtPushButton("restore defaults")
        icon_label = QWidget()
        ...
        self.setWindowTitle(
            Translater.instance().get_translation("restore settings title")
        )
        ...
        layout3 = QVBoxLayout()
        layout3.addWidget(QtLabel("restore settings text"))
    ...
    def accept(self):
        if self.do_not_ask.isChecked():
            ...
            settings.update({"confirm_restore_settings": False})
        super().accept()
