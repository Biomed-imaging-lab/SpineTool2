...
from application.utils.code_syntax_highlight import Pylighter
...
ActionSequence = Sequence[Tuple[str, Callable[["QtNotification"], None]]]


class QtNotification(QDialog):
    ...
    def __init__(
        ...
        Translater.instance().language_changed_signal.connect(
            self.update_message_language
        )
    ...
    def _update_icon(self, severity: str):
        ...
        icon_path = ApplicationSettings.SVGS_PATH + "/" + severity + ".svg"
        icon = QColoredSVGIcon(icon_path)
        self.severity_icon.setPixmap(icon.colored(color=color).pixmap(15, 15))

    def set_message(self, message):
        self._message = message
        self.message.setText(Translater.instance().get_translation(message))

    def update_message_language(self):
    ...
    @classmethod
    def from_notification(cls, notification: Notification, parent: QWidget = None):
    ...
    @classmethod
    @ensure_main_thread
    def show_notification(cls, notification: Notification, parent: QWidget):
        settings = ApplicationSettings.instance()
        if notification.severity >= settings.state["gui_notification_level"]:
            cls.from_notification(notification, parent).show()
...
class TracebackDialog(QDialog):
    def __init__(self, exception, parent=None) -> None:
        ...
        self.resize(650, 270)
        text = QTextEdit()
        theme = get_theme(ApplicationSettings.instance().state["theme"], as_dict=False)
