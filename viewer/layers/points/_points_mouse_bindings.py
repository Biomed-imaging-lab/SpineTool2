...
from viewer.layers.points._points_constants import ActionType
from viewer.layers.points._points_utils import _points_in_box_3d, points_in_box
...
def select(layer, event):
    ...
    while event.type == "mouse_move":
        ...
        if not modify_selection and len(layer.selected_data) > 0:
            is_moving = True
            layer._move(layer.selected_data, coordinates, history, False)
            if history:
                history = False
        else:
    ...
    if is_moving:
        layer._move(layer.selected_data, coordinates, False)
        is_moving = False
