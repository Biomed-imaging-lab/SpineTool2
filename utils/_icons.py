...
def get_colorized_svg(
    ...
    if not svg_elem.search(xml):
        raise ValueError(
            "Could not detect svg tag in {path_or_xml!r}".format(
...
def generate_colorized_svgs(
    ...
    colors : Iterable[Union[str, Tuple[str, str]]]
        ...
        be an available theme name, and the second item
        should be a key in the theme
    ...
    for color, path, op in product(colors, svg_paths, opacities):
        ...
        if isinstance(color, tuple):
            ...
            color = color_as_hex(getattr(get_theme(clrkey, False), theme_key))
