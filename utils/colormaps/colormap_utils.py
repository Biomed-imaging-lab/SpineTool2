...
from utils.colormaps.bop_colors import bopd
from utils.colormaps.colormap import (
    Colormap,
    ColormapInterpolationMode,
    CyclicLabelColormap,
    minimum_dtype_for_labels,
)
from utils.colormaps.inverse_colormaps import inverse_cmaps
from utils.colormaps.standardize_color import transform_color
from utils.colormaps.vendored import cm
...
_VISPY_COLORMAPS_TRANSLATIONS = OrderedDict(
    ...
    fire=("fire", _VCO["fire"]),
    grays=("grays", _VCO["grays"]),
    hot=("hot", _VCO["hot"]),
    ice=("ice", _VCO["ice"]),
    winter=("winter", _VCO["winter"]),
...
BOP_COLORMAPS = {
    name: Colormap(colors=value, name=name, display_name=display_name)
...
def convert_vispy_colormap(colormap, name="vispy"):
    """Convert a vispy colormap object to a colormap.
    ...
    if not isinstance(colormap, VispyColormap):
        raise TypeError("Colormap must be a vispy colormap if passed to from_vispy")
...
def color_dict_to_colormap(colors):
    ...
    if len(control_colors) >= MAX_DISTINCT_COLORS:
        warnings.warn(
            "Label layers with more than {max_distinct_colors} distinct colors will not render correctly. This layer has {distinct_colors}.".format(
...
def vispy_or_mpl_colormap(name):
    ...
    if name in _VISPY_COLORMAPS_TRANSLATIONS:
        ...
        try:
            ...
            if suggestion:
                raise KeyError(
                    'Colormap "{name}" not found in either vispy or matplotlib but you might want to use "{suggestion}".'.format(
                        name=name,
            ...
            raise KeyError(
                'Colormap "{name}" not found in either vispy or matplotlib. Recognized colormaps are: {colormaps}'.format(
                    name=name,
...
AVAILABLE_COLORMAPS_NAMES = AVAILABLE_COLORMAPS.keys()
AVAILABLE_COLORMAPS_NAMES_SHORT_LIST = [
    "gist earth",
    "hsv",
    "inferno",
    "magma",
...
def _increment_unnamed_colormap(
    ...
    if name == "[unnamed colormap]":
        ...
        display_name = "[unnamed colormap {number}]".format(
    ...
    with AVAILABLE_COLORMAPS_LOCK:
        if isinstance(colormap, str):
            ...
            if colormap is not None:
                ...
                warnings.warn(
                    "invalid type for colormap: {cm_type}. Must be a {{str, tuple, dict, Colormap, vispy.colors.Colormap}}. Reverting to default".format(
...
def display_name_to_name(display_name):
    ...
    return display_name_map.get(display_name, list(AVAILABLE_COLORMAPS.keys())[0])
